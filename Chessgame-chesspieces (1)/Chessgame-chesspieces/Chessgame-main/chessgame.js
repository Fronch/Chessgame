...
let changedPawns = 9;
let piecechoiceimg = ["_queen","_king","_rook","_bishop"];
let piecechoice = new Array(4)
printChooser("b")
let horsemoves = [
  { x: 2, y: 1 },
  { x: 2, y: -1 },
...
function backBoard(){
  ctx.fillStyle = "grey"
  ctx.fillRect(offset,offsety,805,805)
  for (let x=0;x<=7;x++){
      for (let y=0;y<=7;y++){
          a = 100*x
          b = 100*y
...
ctx.fillStyle = "grey"
ctx.fillRect(offset + 900, offsety, 150, 805)
  for (let i =0; i<4;i++){
    piecechoice[i] = new Path2D();
    piecechoice[i].rect(offset + 900, offset, 150, 805)
    path = start+piecechoiceimg[i]
    console.log(path)
    let img = pieceImages[path];
    console.log(img)
    ctx.drawImage(img, offset + 890, offset + 10 + i*100, 80, 80)
  }
}
    ...
    for (let x = 0; x < 8; x++) {
      for (let y = 0; y < 8; y++) {
        if (ctx.isPointInPath(positions[x][y],event.offsetX,event.offsetY)) {
          ...
          ctx.strokeRect(100*x+4 + offset,100*y+4 + offsety,96,96)
        }
        else{
