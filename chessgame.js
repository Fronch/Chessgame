...
function printBoard() {
  console.log(topnum)
  for (let y = 0; y < 8; y++) {// prints the divider between each layer
    console.log(divider)
    for (let b = 0; b < 8; b++) {
      row += board[b][y].charAt(0) //prints the value for each point on the board
      row += " | "//changes row to allow for left side, middle, or right side.
    }
    console.log((y + 1), row)
    row = "| "
  }
